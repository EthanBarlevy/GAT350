...
namespace vl
{
	...
	bool Program::Create(std::string name, ...)
	{
		...
		bool success = vl::json::Load(name, document);
		if (!success)
		{
		...
		{
			auto vshader = g_resourceManager.Get<Shader>(vertex_shader, GL_VERTEX_SHADER);
			AddShader(vshader);
		}
		...
		{
			auto fshader = g_resourceManager.Get<Shader>(fragment_shader, GL_FRAGMENT_SHADER);
			AddShader(fshader);
		}
